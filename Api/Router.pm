...
use Base qw(escape_for_sql in_array);
use Api::Validator;
use Api::Paths;
...
sub preprocess {
  ...
  my $Paths = Api::Paths->new($self->{db}, $self->{admin}, $self->{conf}, $self->{lang}, $self->{html});
...
sub handle {
  ...
  if (defined $route->{params}) {
    my $Validator = Api::Validator->new($self->{db}, $self->{admin}, $self->{conf});
...
=head2 parse_request() - parses request and returns data, required to process it
   ...
   Returns:
    {
      route        - hashref of route's info. look at docs in Api::Paths
...
sub parse_request {
  ...
  foreach my $route (@{$resource}) {
    ...
    for my $query_key (keys %{$query_params}) {
      my $key = $route->{no_decamelize_params} ? $query_key : Base::decamelize($query_key);

      if ($key eq 'SORT') {
        $query_params->{$query_key} = Base::decamelize($query_params->{$query_key});
      }
