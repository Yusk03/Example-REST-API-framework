# company own Base function own wrote API functions
#   xml_former
#   escape_for_sql
#   camelize
#   decamelize
...
#**********************************************************
=head2 startup_files($attr) - Get daemon startup information and other params of system
...
sub startup_files {
  ...
  my $startup_conf = $base_dir . '/programs';
  if ( $attr->{TPL_DIR} ) {
...
=head2 escape_from_sql($input) - escapes data so it will be safe to put it to DB functions
  ...
  Same escaping is done in HTML::form_parse.
  This function is meant to be used when HTML::form_parse is not applied.
