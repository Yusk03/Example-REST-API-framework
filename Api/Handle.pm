...
use Base qw(json_former xml_former gen_time in_array check_ip);
...
sub _start {
  ...
  if (!$self->{conf}->{API_ENABLE} && !$self->{direct}) {
    ...
    $router = Api::Router->new($self->{path}, $self->{db}, $self->{admin}, $self->{conf}, $self->{req_params}, $self->{lang}, \@main::MODULES, 0, $self->{html}, $self->{request_method});
    ...
    if ($router->{errno}) {
      ...
      if ($router->{allowed}) {
        $router->transform(\&Api::FieldsGrouper::group_fields);
