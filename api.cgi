...
our $db = SQL->connect($conf{dbtype}, $conf{dbhost}, $conf{dbname}, $conf{dbuser}, $conf{dbpasswd},
...
#**********************************************************
sub _start {
  my $handle = Api::Handle->new($db, $admin, $Conf->{conf}, {
    req_params     => \%FORM,
    html           => $html,
    lang           => \%lang,
    cookies        => \%COOKIES,
  ...
  print JSON::header(undef, { STATUS => $result->{status} || '' });
  print $result->{response};
